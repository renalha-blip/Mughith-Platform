...
const Weather: React.FC<WeatherProps> = ({ incidents }) => {
  ...
  const cityStats = useMemo(() => {
    ...
    return Object.keys(stats).map(city => {
       ...
       // Simulate weather metrics based on context
       ...
       else if (s.weatherCtx === 'إجهاد حراري' || s.weatherCtx === 'موجة حر') { temp = 47; wind = 8; humid = 10; }
      ...
      <div className="glass-panel p-1 rounded-2xl overflow-hidden h-[300px] lg:h-[400px] border border-white/10 relative mb-8">
          ...
          <div className="absolute top-4 right-4 flex flex-col gap-2 z-[1000] items-end">
             <div className="flex items-center gap-2">
                 <button onClick={() => setMapLayer('dust')} className={`px-3 py-1.5 rounded-lg flex items-center gap-2 text-xs font-bold transition-all shadow-lg backdrop-blur-sm ${mapLayer === 'dust' ? 'bg-yellow-600 text-white border border-yellow-500' : 'bg-black/60 text-gray-300 border border-white/10 hover:bg-black/80'}`}>
                     <Wind size={14} /> غبار
                 </button>
                 <button onClick={() => setMapLayer('rain')} className={`px-3 py-1.5 rounded-lg flex items-center gap-2 text-xs font-bold transition-all shadow-lg backdrop-blur-sm ${mapLayer === 'rain' ? 'bg-blue-600 text-white border border-blue-500' : 'bg-black/60 text-gray-300 border border-white/10 hover:bg-black/80'}`}>
                     <CloudRain size={14} /> أمطار
                 </button>
                 <button onClick={() => setMapLayer('heat')} className={`px-3 py-1.5 rounded-lg flex items-center gap-2 text-xs font-bold transition-all shadow-lg backdrop-blur-sm ${mapLayer === 'heat' ? 'bg-red-600 text-white border border-red-500' : 'bg-black/60 text-gray-300 border border-white/10 hover:bg-black/80'}`}>
                     <Sun size={14} /> حرارة
                 </button>
             </div>
          </div>
